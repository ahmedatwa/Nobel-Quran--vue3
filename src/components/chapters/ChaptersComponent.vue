...
import { useAudioPlayerStore, useSurahStore, useTranslationsStore } from "@/stores";
import { ChapterReadingViewComponent, ChapterTranslationsViewComponent } from '@/components';
...
watchEffect(async () => {
    if(props.selected) {
        if(!surahStore.selectedSurah) {
            surahStore.selectedSurah = surahStore.surahList[0]
            await surahStore.getVerses(1, true)
        }
    }
})
...
<template>
    <v-card v-show="selected" class="ma-3" :elevation="1"
        :id="`quran-reader-content-surah${surahStore.selectedSurah?.id}`" :key="surahStore.selectedSurah?.id">
        ...
        <v-card-actions>
            <v-row v-if="surahStore.isLoading.verses">
                <v-col cols="12">
