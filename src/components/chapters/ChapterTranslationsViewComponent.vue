...
const verses = computed(() => {
    if(surahStore.selectedSurah?.verses) {
        return surahStore.selectedSurah?.verses.sort((a, b) => a.verse_number - b.verse_number)
    }
})
...
const setBookmarked = (verseNumber: number) => {
    surahStore.selectedSurah?.verses?.forEach((v) => {
