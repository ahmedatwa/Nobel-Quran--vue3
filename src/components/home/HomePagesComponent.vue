...
const emit = defineEmits<{
    "update:selectedPage": [value: Page]
}>()
...
const getSelected = (value: Page) => {
    pageStore.selectedPage = value  
    emit("update:selectedPage", value)
}

const onMouseEnter = async (page: Page) => {
    ...
    <v-container>
        ...
        <v-row dense v-else>
            <v-col v-for="page in pages" :key="page.pageNumber" cols="12" md="4">
                <v-card @click="getSelected(page)" :border="true" @mouseenter="onMouseEnter(page)">
